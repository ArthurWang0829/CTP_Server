...
public:
	void connect()
	{
		...
		m_mdApi->RegisterFront("tcp://122.51.136.165:20004");
		m_mdApi->Init();
	}
	//登陆
	void login()
	...
	void subscribe()
	{
		...
		ppInstrument[0] = "au2206";
		ppInstrument[1] = "ag2206";
		for (int i = 0; i < 1; i++)
		{
			m_mdApi->SubscribeMarketData(ppInstrument, i+1);
		}
	...
	void OnRtnDepthMarketData(CThostFtdcDepthMarketDataField* pDepthMarketData)
	{
		...
		if (pDepthMarketData)
		{	
			LOG("\tTradingDay = [%s]\t", pDepthMarketData->TradingDay);
			LOG("\tInstrumentID = [%s]\t", pDepthMarketData->InstrumentID);
			LOG("\tExchangeID = [%s]\t", pDepthMarketData->ExchangeID);
			LOG("\tLastPrice = [%.8lf]\t", pDepthMarketData->LastPrice);
			LOG("\tPreSettlementPrice = [%.8lf]\t", pDepthMarketData->PreSettlementPrice);
			LOG("\tOpenPrice = [%.8lf]\t", pDepthMarketData->OpenPrice);
			LOG("\tVolume = [%d]\t", pDepthMarketData->Volume);
			LOG("\tTurnover = [%.8lf]\t", pDepthMarketData->Turnover);
			LOG("\tOpenInterest = [%d]\t", pDepthMarketData->OpenInterest);
			LOG("\tLastUpdateTime = %s\t", pDepthMarketData->UpdateTime);
		}
		LOG("</OnRtnDepthMarketData>\n");
	}
	...
	void sub_qry()
	{
		char** ppinstrument = new char* [50];
		ppinstrument[0] = "au2206";
		while (m_mdApi->SubscribeForQuoteRsp(ppinstrument, 1) != 0) Sleep(1000);
	}
...

{
	...
	// 
	mduser->subscribe();
	LOG(mduser->get_version());
	LOG(mduser->get_trading_day());
	mduser->OnHeartBeatWarning(1);
	//mduser->Register

	CTraderApi* tduser = new CTraderApi;
	pUserApi->CreateFtdcTraderApi(".\\flow\\");
	LOG(pUserApi->GetApiVersion());
	cout << endl;
	logfile = fopen("syslog.txt", "w");
